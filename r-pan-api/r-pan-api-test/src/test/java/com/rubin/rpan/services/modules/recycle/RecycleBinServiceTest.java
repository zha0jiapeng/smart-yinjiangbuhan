package com.rubin.rpan.services.modules.recycle;
...
public class RecycleBinServiceTest {
    ...
    public void restoreSuccessTest() {
        ...
        rPanUserFileVOList = iRecycleBinService.list(userId);
    ...
    public void deleteSuccessTest() {
        ...
        List<Long> fileIdList = rPanUserFileVOList.stream().map(RPanUserFileVO::getFileId).collect(Collectors.toList());
        String fileIds = StringListUtil.longListToString(fileIdList);
        iRecycleBinService.delete(fileIds, userId);
        rPanUserFileVOList = iRecycleBinService.list(userId);
        Assert.assertEquals(0, rPanUserFileVOList.size());
    }
    ...
    private Long deleteFile() {
        ...
        deleteFile(userId, folder.getFileId());
        return userId;
    }
    ...
    private RPanUserFileVO createFolder(Long parentId, Long userId) {
        ...
        iUserFileService.createFolder(parentId, folderName, userId);
        List<RPanUserFileVO> rPanUserFileVOList = iUserFileService.list(parentId, FileConstant.ALL_FILE_TYPE, userId);
    ...
    private void deleteFile(Long userId, Long fileId) {
        iUserFileService.delete(StringListUtil.longListToString(fileId), userId);
    }
