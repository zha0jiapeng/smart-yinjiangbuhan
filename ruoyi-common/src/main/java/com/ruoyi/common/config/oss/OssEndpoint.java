...
package com.ruoyi.common.config.oss;
		...
		if(putObjectResult!=null){
			...
			if(sysFile==null) {//等于空为新增查询不到的文件
				...
				if(StringUtils.isNotEmpty(username)) {
					sysFile=new SysFile(SnowflakeIdWorker.getUUID(),  fileSuffixName,  bucketName, object.getSize(), object.getContentType(),getUserId().toString(), getUsername(), new Date(),null, null, null);
				}else {
	...
	public AjaxResult createObject(@PathVariable("id") String id,@RequestBody MultipartFile object, @PathVariable("bucketName") String bucketName) throws Exception {
		...
		if(oldSysFile!=null){
			...
			if(putObjectResult!=null){
				...
				oldSysFile.setUpdateUserId(getUserId().toString());
				oldSysFile.setUpdateUserName(getUsername());
...
/*
	...
	public Object createObjectLayedit(@RequestBody MultipartFile file, @PathVariable String bucketName) throws Exception {
		...
		if(putObjectResult!=null){
			...
			String username = getUsername();
			if(StringUtils.isNotEmpty(username)) {
				sysFile=new SysFile(uuid,  fileSuffixName,  bucketName, file.getSize(), file.getContentType(),getUserId().toString(), getUsername(), new Date(),null, null, null);
			}else {
