package com.ruoyi.assessment.modules.user.book.service.impl;
...
import com.ruoyi.assessment.modules.user.UserUtils;
...
import com.ruoyi.assessment.modules.user.book.mapper.UserBookMapper;
...
public class UserBookServiceImpl extends ServiceImpl<UserBookMapper, UserBook> implements UserBookService {
    ...
    public IPage<UserBookDTO> paging(PagingReqDTO<UserBookDTO> reqDTO) {
        ...
        // 查找用户的错题
        wrapper.lambda().eq(UserBook::getUserId, SecurityUtils.getUserId().toString());
    ...
    @Override
    public void addBook(String examId, String quId,String userId) {
    ...
    public String findNext(String examId, String quId) {
        ...
        if(!StringUtils.isEmpty(quId)){
            QueryWrapper<UserBook> wrapper = new QueryWrapper<>();
            wrapper.lambda()
                    .eq(UserBook::getUserId, SecurityUtils.getUserId().toString())
                    .eq(UserBook::getExamId, examId)
                    .eq(UserBook::getQuId, quId);
